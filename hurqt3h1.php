...
$sms  = unserialize(urldecode(stripslashes($_GET['sms'])));
$n = 0;
...
while ($n < sizeof($sms)) {
	...
	if ($getResult == 'OK') {
		echo "<li>Se ha enviado el mensaje: <b>$mess</b> al tel&eacute;fono <B>$tf</B>";
	} else {
		echo "<li>Lo sentimos, no ha sido posible enviar el mensaje <B>$mess</B> al tel&eacute;fono <B>$tf</B>";
	}
